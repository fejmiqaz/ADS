...
public class Main {
    public static void main(String args[]) {
        ...
        String[] startCities = new String[numPairs];
        String[] endCities = new String[numPairs];

        for (int i = 0; i < numPairs; i++) {
            startCities[i] = scanner.next();
            endCities[i] = scanner.next();
        }

        int numEdges = scanner.nextInt();
        AdjacencyListGraph<String> cityGraph = new AdjacencyListGraph<>();

        for (int i = 0; i < numEdges; i++) {
            String sourceCity = scanner.next();
            String destinationCity = scanner.next();
            int edgeWeight = scanner.nextInt();
            cityGraph.addEdge(sourceCity, destinationCity, edgeWeight);
        }

        int totalShortestPathCost = 0;

        for (int i = 0; i < numPairs; i++) {
            totalShortestPathCost += cityGraph.shortestPath(startCities[i]).get(endCities[i]);
        }

        System.out.println(totalShortestPathCost);
    }
}
