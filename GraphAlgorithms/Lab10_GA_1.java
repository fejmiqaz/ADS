...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numRoads; i++) {
            ...
            cityGraph.addEdge(city1, city2, roadCost);
        }

        int totalCost = 0;
        Set<String> visitedCities = new HashSet<>();

        String startingCity = cities[0];
        ...
        while (visitedCities.size() < numCities) {
            ...
            for (String currentCity : visitedCities) {
                // Get all its neighbors and their distances
                Map<String, Integer> neighbors = cityGraph.getNeighbors(currentCity);

                for (Map.Entry<String, Integer> neighbor : neighbors.entrySet()) {
                    String neighborCity = neighbor.getKey();
                    int connectionCost = neighbor.getValue();

                    if (!visitedCities.contains(neighborCity) && connectionCost < minimumCost) {
                        minimumCost = connectionCost;
